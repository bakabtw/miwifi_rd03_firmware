...
wandt_setup_wan() {
    ...
    ps_setup_wan "wan" "$wan_proto" "$wan_port"
    wandt_filter_kmod close

    local automode=$(uci -q get ipv6.wan6.automode)
    [ "$automode" = "1" ] && /usr/sbin/ipv6.sh autocheck wan6 clear_result &
    return
}
...
wandt_prog_control() {
    ...
    wan_detect_pid=$(pgrep wan_detect)
    [ -n "$wan_detect_pid" ] && {
